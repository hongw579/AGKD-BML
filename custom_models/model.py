...
class WideResNet(nn.Module):
    ...
    def forward(self, x, _eval = False):
        ...
        x4 = F.avg_pool2d(feature, 8)
        x4 = x4.view(-1, self.nChannels)
#        x4 = self.fc(x4)
#        self.train()
        return feature, x4, self.fc(x4)

class SupConResNet(nn.Module):
    """backbone + projection head"""
    def __init__(self, basic_net, head='mlp', dim_in = 640, feat_dim=128):
        super(SupConResNet, self).__init__()
        self.encoder = basic_net
        if head == 'linear':
    ...
    def forward(self, x):
        features, x4s, logits = self.encoder(x)
        feat = F.normalize(self.head(x4s), dim=1)
        return features, x4s, logits, feat
